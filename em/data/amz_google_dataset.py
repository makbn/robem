...
class AmazonGoogleDataset(EmBaseDataset):
    ...
    def _do_aug(self, label_row):
        ...
        aug_pairs = []
        for lc in left_augs:
            new_left = (lc.replace('[ UNK ]', '[UNK]'), left_can[1], left_can[2])
            for rc in right_augs:
                new_right = (rc.replace('[ UNK ]', right_can[1], '[UNK]'), right_can[2])
    ...
    def __init__(self, path, mode='train', transform=None, device=None, sentence_size=256, da=False, ditto_aug=None):
        ...
        self._read_tabel('tableA.csv', 'left', ['title', 'manufacturer', 'price'])
        self._read_tabel('tableB.csv', 'right', ['title', 'manufacturer', 'price'])
        if da:
