...
class DblpAcmDataset(EmBaseDataset):
    ...
    def __init__(self, path, mode='train', transform=None, device=None, sentence_size=256, da=True, ditto_aug=None):
        super().__init__(path, mode, transform, device, sentence_size)
        self._read_tabel('tableA.csv', 'left', ['title', 'authors', 'venue', 'year'])
        self._read_tabel('tableB.csv', 'right', ['title', 'authors', 'venue', 'year'])
        if da:
