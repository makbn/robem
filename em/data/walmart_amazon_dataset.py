...
class WalmartAmazonDataset(EmBaseDataset):
    ...
    def _do_aug(self, label_row):
        ...
        aug_pairs = []
        for lc in left_augs:
            new_left = (lc.replace('[ UNK ]', '[UNK]'), left_can[1], left_can[2], left_can[3], left_can[4])
    ...
    def __init__(self, path, mode='train', transform=None, device=None, sentence_size=256, da=False, ditto_aug=None):
        ...
        self._read_tabel('tableA.csv', 'left', ['title', 'category', 'brand', 'modelno', 'price'])
        self._read_tabel('tableB.csv', 'right', ['title', 'category', 'brand', 'modelno', 'price'])
        if da:
