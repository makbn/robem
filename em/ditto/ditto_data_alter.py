...
class DittoItunesAmazonAlter:
    ...
    def alter(self, left, right):
        ...
        for i in range(max_len):
            col_name = f'EXP-{i}' if i >= len(all_cols) else all_cols[i]
            l_v = ' ' if i >= len(left_values) else left_values[i]
            r_v = ' ' if i >= len(right_values) else right_values[i]
            l_str += f"COL {col_name} VAL {l_v} "
            r_str += f"COL {col_name} VAL {r_v} "
